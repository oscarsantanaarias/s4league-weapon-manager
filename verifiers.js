      ...
      const findName = new RegExp(`value${i}="${name}"`, 'g');
        const findID = new RegExp(`<weapon item_key="(${id})">`, 'g');
                ...
                if(matchesName.length > 0){
                    console.error('Name is already in use at Weapons.xml');
                    return resolve([false, 2]);
                }

                if(matchesID.length > 0){
                    console.error('ID is already in use at Weapons.xml');
                    return resolve(false);
                }
...
module.exports = {verifyFields, verifyInfox7, verifyItemX7, verifyWeaponLua, verifyItem_xml , verifyWeaponsXML ,iteminfox7 , itemxml,  itemx7, weaponlua, weaponxml, melee, id_range, special, sentries, guns, snipers, heavies, thrown};